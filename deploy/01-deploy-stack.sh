...
deploy_dse(){
  helm install -f ./dse/cass-operator-values.yaml cass-operator datastax/cass-operator
  sleep 5
  kubectl apply -f ./dse/retailcdc-secret.yaml
  kubectl apply -f ./dse/deploy-cassandra.yaml
  sleep 60
  kubectl apply -f ./dse/stargate-deploy.yaml
